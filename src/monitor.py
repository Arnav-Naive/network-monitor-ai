...
def save_to_csv(data, anomalies):
    ...
    with open('data/logs.csv', 'a', newline='') as f:
        writer = csv.writer(f)
        if not file_exists:
            writer.writerow(['timestamp', 'cpu_usage', 'temperature', 'bandwidth', 'port_status', 'anomalies'])
        writer.writerow([
            data['timestamp'],
            data['cpu_usage'],
            data['temperature'],
...
try:
    while True:
        ...
        print(f"{data['timestamp']} | CPU: {data['cpu_usage']}% | Temp: {data['temperature']}C | Bandwidth: {data['bandwidth']}Mbps | Port: {'UP' if data['port_status'] else 'DOWN'}")
